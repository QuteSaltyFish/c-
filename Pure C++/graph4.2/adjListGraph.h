...
void adjListGraph<TypeOfVer, TypeOfEdge>::prim(TypeOfEdge noEdge)const
{
    ...
    cout<<"Prim:\n";
    bool *flag = new bool[this->Vers];  //store whether the node is in the SET U
    TypeOfEdge *lowCost=new TypeOfEdge[this->Vers];     //represent the lowest weight of edge from the node in U to v-u
    int *startNode=new int[this->Vers]; //store the node from u to node i whose weight is lowCost[i]
    ...
    {
        for (p=verList[start].head; p!=nullptr; p=p->next)  //compare the new added node's edge weight with the previous lowest weight
        {
            if (!flag[p->end] && lowCost[p->end]>p->weight)
            {   //upadte the distance information
                lowCost[p->end] = p->weight;    //lowest weight to node p->end
                startNode[p->end] = start;  //the node of which the edge to p->end is the lowest edge to p->end
            }
        }
        flag[start] = true;     //add the origin node to the set u
        min = noEdge;
        ...
        out<<'('<<verList[startNode[start]].ver<<','<<verList[start].ver<<", the Value is:"<<min<< ")\t";
        cout<<'('<<verList[startNode[start]].ver<<','<<verList[start].ver<<", the Value is:"<<min<< ")\t";
