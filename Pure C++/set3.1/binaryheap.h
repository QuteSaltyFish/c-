...
void priorityQueue<T>::enQueue(const T&x)
{
    ...
    {
        time[0]++;
        array[hole] = array[hole/2];//find upwords until the parent node is smaller than new node;
    }
    time[1]++;
    array[hole]=x;
}
...
template <class T>
priorityQueue<T>::priorityQueue(const T *item, int size):maxSize(size+10), currentSize(size)
{
    time = new int[2]{0};
    array = new T[maxSize];
    for (int i=0; i<size; ++i)
